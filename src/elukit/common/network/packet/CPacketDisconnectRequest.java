...
public class CPacketDisconnectRequest extends Packet {
	public CPacketDisconnectRequest(){}
	...
	public void onReceive(Side side, DatagramPacket raw) {
		if (side == Side.SERVER){
			...
			if (Server.connections.contains(sockaddr)){
				Server.console.addText("Disconnected client "+sockaddr.toString()+".");
				Server.connections.remove(sockaddr);
				NetworkHelper.sendPacket(Server.socket, raw.getAddress(), raw.getPort(), new SPacketClientDisconnect());
			}
		}
	}
